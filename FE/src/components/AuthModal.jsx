...
/** Client ID lấy từ .env (VITE_GOOGLE_CLIENT_ID), không hardcode */
...
export function AuthModal({ isOpen, onClose, onLogin }) {
  ...
  const handleGoogleCredential = async (response) => {
    ...
    try {
      ...
      const userInfo = buildUserInfo(res);
      if (res.token) userInfo.token = res.token;
  ...
  // Gắn nút Google (API thật) lên vùng ẩn đè trên nút custom khi có cấu hình
  useEffect(() => {
    if (!isOpen || (mode !== MODE.LOGIN && mode !== MODE.REGISTER) || !GOOGLE_CLIENT_ID) return;
    const wrapper = googleButtonRef.current;
    if (!wrapper) return;
    const el = wrapper.querySelector(".google-signin-invisible");
    if (!el) return;
    el.innerHTML = "";
    const init = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === MODE.RESET) {
      if (
        ...
        setErrorMessage(
          "Vui lòng dán mã đặt lại và nhập mật khẩu mới (ít nhất 6 ký tự).",
        );
        return;
      }
      try {
        ...
        setErrorMessage(
          err.message || "Mã đặt lại không hợp lệ hoặc đã hết hạn.",
        );
      }
      return;
    }
  ...
  const handleGoogleClickWhenNotConfigured = () => {
    if (!GOOGLE_CLIENT_ID) {
      setErrorMessage(
        "Chưa cấu hình đăng nhập Google. Thêm VITE_GOOGLE_CLIENT_ID vào file FE/.env (và BE/.env GOOGLE_CLIENT_ID).",
      );
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {mode === MODE.FORGOT && (
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Nhập email để nhận mã đặt lại mật khẩu (mã hiển thị ngay sau
                  khi gửi).
                </p>
              </div>
            )}
            {mode === MODE.FORGOT && forgotSuccess && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-xl text-sm text-green-800">
                Mã đặt lại đã được tạo. Sao chép mã bên dưới rồi bấm &quot;Đặt
                lại mật khẩu&quot; để nhập mã và mật khẩu mới.
                {formData.resetToken && (
                  <p className="mt-2 font-mono text-xs break-all bg-white p-2 rounded">
                    {formData.resetToken}
                  </p>
                )}
                <button
                  ...
                  className="mt-3 w-full py-2 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700"
                >
                  Đặt lại mật khẩu
                </button>
              </div>
            ...
            >
              {isLocked
                ...
                : (mode === MODE.LOGIN && "Đăng nhập") ||
                  (mode === MODE.REGISTER && "Đăng ký") ||
                  (mode === MODE.FORGOT && "Gửi mã đặt lại") ||
                  (mode === MODE.RESET && "Đặt lại mật khẩu")}
            </button>
          </form>
          ...
          {(mode === MODE.LOGIN || mode === MODE.REGISTER) && (
            <>
              ...
              <div className="space-y-3">
                <div
                  ref={googleButtonRef}
                  className="relative w-full max-w-[320px] min-h-[44px] mx-auto"
                >
                  {/* Giữ form đẹp: nút custom giống ban đầu */}
                  <div
                    className={`absolute inset-0 flex items-center justify-center gap-3 px-4 py-3 border-2 rounded-xl font-semibold transition-colors pointer-events-none ${
                      isLocked
                        ? "border-gray-200 bg-gray-100 text-gray-400"
                        : "border-gray-300 text-gray-700 bg-white hover:bg-gray-50 border-[#4285F4]/30"
                    }`}
                  >
                  ...
                  {/* Vùng nhận click: nút Google API (ẩn) hoặc fallback khi chưa cấu hình */}
                  {GOOGLE_CLIENT_ID ? (
                    <div
                      className="google-signin-invisible absolute inset-0 flex items-center justify-center opacity-0 min-h-[44px] cursor-pointer"
                      aria-hidden="true"
                    />
                  ) : (
                    <button
                      type="button"
                      onClick={handleGoogleClickWhenNotConfigured}
